<?php
function form_focus_output_option( $option ) {

	$current 	= formfocus_get_option( $option['name'] );
	...
	switch( $field_type ):
			...
			<fieldset>
				<?php foreach( $option['options'] as $val => $choice ):
					if( $val == $current )
						...
						$selected = ''; ?>
					<label for="<?php echo $prefix.$option['name'].$val; ?>">
						<input type="radio" name="<?php echo $option['group'].'['.$option['name'].']'; ?>" value="<?php echo $val; ?>" id="<?php echo $prefix.$option['name'].$val; ?>" <?php echo $selected; ?> />
		...
		case 'range': ?>
			...
			<input type="range" name="<?php echo $option['group'].'['.$option['name'].']'; ?>" id="<?php echo $prefix . '_' . $option['name']; ?>" value="<?php echo $current; ?>" class="overlayOpacity" min="0" max="100" step="5" />
			<output for="overlayOpacity" class="overlayOpacityVal">75</output>
			<?php if( isset($option['desc']) && $option['desc'] != '' ): ?><p class="description"><?php echo $option['desc']; ?></p><?php endif; ?>
